...
class Appr(Inc_Learning_Appr):
    ...
    def train_epoch(self, t, trn_loader, e):
        ...
        for images, targets in trn_loader:
            ...
            loss = self.criterion(t, outputs, targets.to(self.device), outputs_old, kd = kd, weight = weight)
            # Backward
    ...
    def criterion(self, t, outputs, targets, outputs_old=None,  kd = False, weight = 1):
        """Returns the loss value"""

        # Classification loss for new classes
        loss = torch.nn.functional.cross_entropy(torch.cat(outputs, dim=1), targets)
        # Distilation loss
        if t > 0 and outputs_old and kd == True:
            ...
            for i in range(last_head_idx):
                loss += weight * self.lamb * F.binary_cross_entropy(F.softmax(outputs[i] / self.T, dim=1),
